...
class TurnoController extends Controller
{
    ...
    public function index(IndexTurno $request)
    {
        ...
        $data = AdminListing::create(Turno::class)->processRequestAndGet(
            ...
            ['id', 'nombre_turno', 'sigla', 'orden', 'hora_inicio', 'hora_fin', 'sesion_clase_id', 'sesion_clase.nombre_sesion_clase'],

            // set columns to searchIn
            ['id', 'nombre_turno', 'sigla', 'orden', 'sesion_clase.nombre_sesion_clase'],

            function ($query) use ($request) {
                $query->with(['sesion_clase']);

                // add this line if you want to search by author attributes
                $query->join('sesion_clase', 'sesion_clase.id', '=', 'turno.sesion_clase_id');

                if($request->has('turno')){
                    $query->whereIn('turno_id', $request->get('turno'));
                }
            }
        );
