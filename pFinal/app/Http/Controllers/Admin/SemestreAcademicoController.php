...
class SemestreAcademicoController extends Controller
{
    ...
    public function index(IndexSemestreAcademico $request)
    {
        ...
        $data = AdminListing::create(SemestreAcademico::class)->processRequestAndGet(
            ...
            ['id', 'nombre_semestre', 'orden', 'sesion_clase_id', 'sesion_clase.nombre_sesion_clase'],

            // set columns to searchIn
            ['id', 'nombre_semestre', 'orden', 'sesion_clase.nombre_sesion_clase'],

            function ($query) use ($request) {
                $query->with(['sesion_clase']);

                // add this line if you want to search by author attributes
                $query->join('sesion_clase', 'sesion_clase.id', '=', 'semestre_academico.sesion_clase_id');

                if($request->has('semestre_academico')){
                    $query->whereIn('semestre_academico_id', $request->get('semestre_academico'));
                }
            }
        );
    ...
    public function create()
    {
        ...
        return view('admin.semestre-academico.create')
